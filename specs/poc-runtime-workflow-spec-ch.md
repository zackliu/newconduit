...
Web PubSub 的 POC 形态是：central、client、sidecar 都作为 Web PubSub clients 连接。Web PubSub 不启用 upstream，不要求 central 暴露公网 callback endpoint，也不把 Web PubSub 当作业务处理方。它只提供一条能跨网络保持的长连接；central 收到事件后写本地 storage，再通过自己的 Web PubSub client connection 把结果发到 session group 和 worker group。

## 4. Web PubSub 映射

POC 使用 hub `agent-runtime-poc`。
...
| Central event inbox | group `central:events`，client/sidecar 的 runtime events 先发到这里。 |
| Session event stream | group `session:{sessionId}`。 |
| Worker command channel | group `worker:{workerId}`。 |
| Client writes | client publish 到 `central:events`，payload 中包含 session/action/correlation 信息。 |
| Sidecar writes | sidecar publish 到 `central:events`，payload 中包含 worker/session/lease generation 信息。 |
| Central fan-out | central 作为 Web PubSub client publish 到 `session:{sessionId}` 和 `worker:{workerId}`。 |

Group membership 由 central 在 negotiate 时通过 token claims 控制；POC 不让 client 和 sidecar 自行加入任意 session/worker group。`central:events` 是 central-owned inbox，client/sidecar 只拿到 POC 所需的最小发送权限。

POC 使用 central 本地配置中的 demo principal 生成 Web PubSub token；`/negotiate` 仍然是 central-owned boundary。Browser 和 sidecar 不自行拼 `userId`，create session payload 也不携带 `tenantId`、`principal`、`owner`。POC 文档和代码标注该身份方案是 demo-only identity。
...
Create session 的语义是 client 请求当前 tenant runtime 创建 durable session。POC 的传输实现固定为：client publish `session.create.requested` 到 `central:events` group。Payload 只描述要创建什么 session，不自报 tenant 或 principal：`agent.agentSpecId`、`input.initialMessage`、`input.clientRequestId`、`workspace.source`。Central 从 Web PubSub negotiate/connection context 得到 demo principal，从启动配置得到 tenant `poc`，然后创建 `session.json`，再 append `session.created` 和 initial input event。
...
Central publish `session.assign` 到 `worker:{workerId}` group。Sidecar 收到后挂载 workspace volume 和 Copilot session volume，并按 AgentSpec launch 启动 Copilot process。

### 7.4 Run Session

Client input 通过 Web PubSub publish 到 `central:events`。Central 的 Web PubSub client connection 收到后 append input event，再 publish 到 `worker:{workerId}`。Sidecar 转给 Copilot process。

Agent output、tool event、status event 由 sidecar publish 到 `central:events`。Central append event 后，publish 到 `session:{sessionId}`。Client 从 session group 接收 stream。

### 7.5 Pause Session

Pause session 的语义是 client 请求 central 把 running session 带到 durable pause boundary。POC 的传输实现固定为：client publish `session.pause.requested` 到 `central:events` group。Central append `pause.requested`，把 session 状态改为 `pausing`，并 publish pause command 到 worker group。
...
Resume session 的语义是 client 请求 central 从 paused session 恢复执行投影。POC 的传输实现固定为：client publish `session.resume.requested` 到 `central:events` group。Central 读取 session、latest snapshot、resolved AgentSpec、pausePolicy、agentStatePolicy 和 recoveryPolicy。POC 不处理 crash recovery，但 resume 仍然走 Recovery controller 的 planned path。
...
| Web PubSub transport adapter | 统一处理 central/client/sidecar client connection、negotiate、group membership 和 group publish。 |
