...
test('scenario: Web PubSub runtime channel mapper produces tenant-prefixed groups', () => {
  ...
  assert.equal(mapper.toGroup({ kind: 'client-inbox' }), 'tenant:tenant%2Fa:clients');
  assert.equal(mapper.toGroup({ kind: 'client-private-inbox', clientConnectionId: 'client/a' }), 'tenant:tenant%2Fa:client:client%2Fa:inbox');
