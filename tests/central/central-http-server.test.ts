...
test('scenario: central starts server with health and negotiate endpoints', async () => {
  ...
  const centralService = new CentralService({ transport: webPubSubTransportAdapter });
  ...
  try {
    ...
    assert.match(clientToken.url, /central%3Aevents/);
    ...
    assert.match(sidecarToken.url, /central%3Aevents/);
  } finally {
    await server.close();
  }
});