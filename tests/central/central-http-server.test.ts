...
test('scenario: central starts server with health and negotiate endpoints', async () => {
  ...
  try {
    ...
    const clientNegotiateResponse = await fetch(`http://localhost:${port}${POC_RUNTIME_HTTP_PATHS.clientNegotiate}?${POC_RUNTIME_HTTP_QUERY.tenantId}=poc`, { method: 'POST' });
    ...
    const sidecarNegotiateResponse = await fetch(`http://localhost:${port}${POC_RUNTIME_HTTP_PATHS.sidecarNegotiate}?${POC_RUNTIME_HTTP_QUERY.tenantId}=poc`, { method: 'POST' });
