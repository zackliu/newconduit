...
- Demo AgentSpec, WorkerPool, and host-pool-controller definitions are declarative JSON documents under `config/` (`agent-specs/`, `worker-pools/`, `host-pool-controllers/`) and are the default startup config source (`CONFIG_DIR` defaults to `config`); the sidecar container ships them via `COPY config ./config`. Worker types are image-declared code build profiles in `src/sidecar/worker-types.ts` (they only name the adapter combination), not config. Matching is pure labels: an AgentSpec's `workerSelector.matchLabels` (including a conventional `storage` capability label) is matched against worker labels, there is no `sidecarClass`; worker labels + capacity are declared once on `WorkerPool.template`. The concrete storage driver is code — a worker reports its `storageClass` at registration and central records it — and snapshots/`workspaceRef` are opaque handle envelopes with no filesystem path or backend prefix. `src/` holds no hardcoded AgentSpec/WorkerPool definitions.
